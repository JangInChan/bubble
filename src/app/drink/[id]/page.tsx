  ...
  useEffect(() => {
    async function fetchDrink() {
      try {
        ...
        // 별점 가져오기 - .data로 접근하고 에러 처리 추가
        try {
          const scoresResponse = await getDrinkScores(drinkId);
          const scoreValue = scoresResponse.data;
          setScore(
            typeof scoreValue === "number" && !isNaN(scoreValue)
              ? scoreValue
              : 0
          );
        } catch (scoreError) {
          console.error("별점 조회 실패:", scoreError);
          setScore(0);
        }

        // 리뷰 가져오기 - .data로 접근하고 에러 처리 추가
        try {
          const reviewsResponse = await getDrinkReviews(drinkId);
              ...
              <div className="flex">
                {Array.from({ length: 5 }).map((_, index) => (
                  <Image
                    key={index}
                    src={
                      index < Math.round(score) ? "/star.svg" : "/nonestar.svg"
                    }
                    alt={index < Math.round(score) ? "별점" : "빈 별점"}
                    width={20}
                    height={20}
                  />
                ))}
                    ...
                    <div className="flex mb-2">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Image
                          key={i}
                          src={
                            i < Math.round(Number(review.score) || 0)
                              ? "/star.svg"
                              : "/nonestar.svg"
                          }
                          alt={
